...
# List of columns
memory_transpose = [''.join(letter) for letter in (lst_str_to_array(memory_text).T.tolist())]
...
memory_transpose_right_copy = [''.join(letter) for letter in (lst_str_to_array(memory_text_right_copy.copy()).T.tolist())]
memory_transpose_right_copy.pop(0)
memory_transpose_diag_right = get_diagonal_string(memory_transpose_right_copy)

xmas_count = 0
for lst in [memory_text,memory_transpose,memory_text_diag_left,memory_transpose_diag_left,memory_text_diag_right,memory_transpose_diag_right]:
